...
test("renderHandoffPrompt embeds the compaction checkpoint verbatim", () => {
  ...
  assert.match(rendered, /## Context checkpoint \(pi-style compaction summary\)/);
...
test("assisted handoff is generated by the compaction pipeline end-to-end", async () => {
  ...
  try {
    ...
    assert.match(cont.run.inputInstruction!, /## Context checkpoint \(pi-style compaction summary\)/);
