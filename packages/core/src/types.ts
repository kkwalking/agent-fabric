...
export interface HandoffContent {
  ...
  /**
   * pi-style compaction checkpoint (structured summary + file XML tags)
   * generated by the LLM summarization pipeline (core/compaction.ts).
   * When present, the rendered handoff prompt embeds it verbatim — the
   * mapped fields above are for UI/inspection. Absent on harness-
   * generated and heuristic fallback handoffs.
   */
