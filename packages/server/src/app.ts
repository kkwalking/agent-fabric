...
export async function createApp(options: ServerOptions): Promise<Express> {
  ...
  app.get("/api/handoffs/:id", (req, res) => {
    ...
    // checkpoint, not independent data — re-derive them at read time so
    // handoffs generated by older parsers inspect consistent with the
    // current one (no LLM involved).
    ...
    ok(res, { ...h, content, renderedPrompt: renderHandoffBody(h), consumedByRunIds });
  });
